...
// point revient à faire disparaître tout Roissy dès que sa référence sort du
// cadre — ce qui arrive au premier panoramique dès le zoom 14.
//
// On élargit donc l'emprise du rayon propre au terrain avant de l'y chercher.
// Mesuré sur la base en service, la distance d'une référence à l'extrémité de
// piste la plus lointaine vaut 521 m en médiane, 4 km au 99,9e centile et
// 10 km au 99,99e — d'où un rayon calculé par terrain plutôt qu'une marge
// forfaitaire, qui trahirait aux deux bouts : trop courte pour les grandes
// plates-formes, trop large partout ailleurs.
...
// ------------------------------------------------------------
//
// Le cache d'aéroports ne retient d'une piste que le cap, la longueur et la
// surface — tout ce que la carte en montre. Le tour de piste, lui, se construit
// sur les COORDONNÉES DES DEUX SEUILS : le sens d'atterrissage et l'axe de piste
// en découlent. La donnée est dans le fichier, mais la garder en mémoire pour
// les 41 500 terrains qui en ont doublerait le cache — payer le monde entier à
// chaque lancement pour un circuit tracé sur un terrain à la fois.
//
// On relit donc le fichier à la demande, EN BINAIRE : recherche d'octets de
// `"ident":"LFMD"`, puis décodage et analyse de CETTE ligne seule. Mesuré sur
// la base en service (118 Mo, 85 743 terrains) : ~55 ms par appel, presque
// entièrement de la lecture — sans décoder les 118 Mo en UTF-8 ni reparser une
// ligne de plus que celle cherchée. C'est l'ouverture d'une modale, pas une
// boucle de rafraîchissement : la relecture ne coûte rien de perceptible.
const OCTET_LF = 10;
...
//     déplacement de carte. Vingt relectures de 70 Mo par panoramique seraient
//     absurdes : il faut un index.
//
// Mesuré sur la base en service : l'index pèse 24 Mo (le cache des aérodromes
// passe de 24 à 49 Mo) et se bâtit en ~770 ms. Il n'est donc monté QUE si l'une
// des deux couches est allumée — qui ne trace ni pistes ni places ne paie rien.
// Même parti que l'index de recherche. Ensuite, une emprise coûte ~7 ms.
let _pistes = null;   // Map ident -> { pistes, rayonPistesM, rayonParkingsM }
...
// — et évite d'avoir à DEVINER une marge forfaitaire, qui serait fatalement
// trop courte pour une grande plate-forme et trop large pour tout le reste.
...
//
// Mesuré sur la base en service : ~18 ms par déplacement de carte, cache tiède.
// Plafond de terrains lus en une fois. Il ne vide plus la couche quand il est
...
// tous des hélistations. Les écarter d'un bloc éteignait les places de LAX,
// c'est-à-dire précisément celles qu'on regardait.
const PARKINGS_MAX_TERRAINS = 24;

// Places déjà lues, par terrain. Au zoom 15 on tourne autour d'UN terrain : sans
// ce cache, chaque panoramique relirait le fichier de 70 Mo. Plafonné, car un
// grand terrain pèse quelques centaines de places.
const PARKINGS_CACHE_MAX = 32;
const _parkingsCache = new Map();   // ident -> tableau de places (vide = terrain sans place)
...
function parkingsDansBbox(bbox) {
  ...
  // que la référence du terrain sort du cadre. Le rayon des pistes sert de
  // base — les aires bordent le faisceau — relevé au plancher ci-dessus.
  //
  // L'index des pistes est bâti au besoin. C'est 470 ms et 21 Mo, mais qui
  // regarde les places d'une plate-forme regarde ses pistes juste avant.
  const idx = chargerIndexPistes();
  let vises = [];
  for (const a of all) {
    const e = idx.get(a.ident);
...
function reload() {
  _airports = null; _navaids = null; _index = null; _pistes = null;
  _parkingsCache.clear();
  _sansPlaces = null;
}

module.exports = { aeroportsDansBbox, navaidsDansBbox, aeroportParCode, detailsAeroport, pistesDansBbox, parkingsDansBbox, rechercherLieux, featureProche, reload };
