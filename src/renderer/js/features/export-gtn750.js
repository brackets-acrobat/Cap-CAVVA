...
//
// Le GTN750 n'accepte qu'un format — « Only the PLN format is accepted » —,
// c'est-à-dire le XML SimBase.Document hérité de FSX. Le gabarit suivi ici est
// celui du fpl.pln d'exemple livré avec le paquet, pas une reconstitution.
//
...
// result in a CTD »). Or les points de report VFR français sont exactement
// cela — NE, SIERRA, MM-CV n'existent qu'attachés à leur terrain. Un point
// tournant part donc SANS bloc ICAO, avec ses seules coordonnées : le GTN
// affiche un point utilisateur là où Cap CAVVA le montre, et la géométrie de la
// route est celle de la carte au mètre près. Seuls le départ et l'arrivée
// gardent leur identité d'aérodrome — cas normal, et sûr.
//
// ── L'altitude portée par un point ──────────────────────────────────────────
// Cap CAVVA planifie une altitude par branche, le PLN en porte une par point.
// On écrit sur un point l'altitude de la branche qui y ARRIVE : c'est celle à
// laquelle on le franchit réellement. Départ et arrivée portent, eux, l'altitude
// du terrain.
// ============================================================
...
function identGtn(nom, pris) {
  let base = sansAccents(String(nom == null ? '' : nom)).toUpperCase().replace(/[^A-Z0-9]/g, '');
  if (!base) base = 'WPT';
...
async function ficheAeroportPln(code) {
  ...
  try {
    const res = await window.cap.aeroportParCode(code);
...
async function construirePln() {
  ...
  // l'altitude planifiée de la branche voisine, seul chiffre dont on dispose.
  ...
  // L'altitude de croisière annoncée est la plus haute des branches.
  ...
  routeWaypoints.forEach((p, i) => {
    // Altitude de la branche qui ARRIVE sur ce point : la branche i.
...
exportGtnBtn.addEventListener('click', async () => {
  ...
  if (!pln) { messageDoc(t('gtnExportVide'), false); return; }

  let res;
  try { res = await window.cap.exporterGtn750({ pln }); }
  catch (err) { res = { ok: false, raison: 'ecriture', error: (err && err.message) || String(err) }; }

  if (res && res.ok) {
    confirmerExportGtn();
    messageDoc(t('gtnExportOk'), false);
    return;
  }
  ...
  messageDoc(t(cles[raison] || 'gtnExportErr').replace('{err}', (res && res.error) || '?'), false);
});
