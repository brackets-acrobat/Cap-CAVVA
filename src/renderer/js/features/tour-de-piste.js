...
// ============================================================
//
// Repris de NavXpressVFR (traffic-pattern.js). Clic droit sur un terrain →
// « Tracer un tour de piste » → une modale demande la piste (donc le sens
// d'atterrissage), la main du circuit, la longueur des branches et l'altitude.
// À la validation, le circuit apparaît en rouge, avec le cap MAGNÉTIQUE de
// chaque branche, une pointe de flèche dans le sens du vol, et l'altitude au
// centre. Plusieurs circuits coexistent ; chacun s'efface par clic droit sur
// son tracé, comme les cercles de portée et les flanquements.
//
...
//
//  • La géométrie de base vient de route.js (capVraiInitial, pointADistance,
//    angleEcranPourCap, wrapLon) plutôt que d'être recopiée, comme pour le
//    flanquement. La couche « pistes » s'en sert aussi.
//
//  • Pas de modale de confirmation avant une suppression. NavXpressVFR en pose
//    deux ; Cap CAVVA n'en pose ni pour les cercles de portée, ni pour les
//    mesures, ni pour les flanquements. Un tracé se refait en trois clics.
//
//  • Pas d'annonce vocale « vent arrière » : Cap CAVVA n'a aucun son. Le
//    simulateur fournit pourtant déjà tout ce qu'il faudrait (cap vrai,
//    altitude MSL, hauteur-sol) — l'ajout reste ouvert.
//
// Les circuits partent dans le plan de vol (.ccfp) : les paramètres saisis, les
// deux seuils et la déclinaison. Rien n'y est recalculé à la lecture — un
// circuit tracé est un choix de l'organisateur, pas une donnée dérivée.
// ============================================================
...
// dernière saisie — on trace rarement un seul circuit dans une séance.
...
// Appelé par initMap. Les deux couches ne sont PAS ajoutées d'emblée : leur
// présence sur la carte est précisément ce qui porte le seuil de zoom.
...
async function ouvrirModaleTourDePiste(airport) {
  ...
  let res = null;
  try { res = await window.cap.detailsAeroport(airport.ident); } catch (_) { res = null; }
...
async function validerTourDePiste() {
  ...
  try {
    const rd = await window.cap.declinaison(terrain.lat, wrapLon(terrain.lon));
...
// Ce qui part dans le .ccfp : les paramètres saisis et les deux seuils. La
// déclinaison EST enregistrée, contrairement au flanquement — un tour de piste
// relu doit porter les caps qui ont servi au brief, et non ceux du jour où on
// le rouvre.
