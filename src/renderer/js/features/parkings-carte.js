...
// ============================================================
//
// Une pastille par place, À L'ÉCHELLE elle aussi : son rayon est celui que MSFS
// déclare pour l'emplacement (`radius_m`), de sorte qu'un poste gros-porteur se
// distingue d'un plot d'aviation générale sans qu'on ait à le dire. L'infobulle
// porte l'identifiant — « GATE A 12 », « PARKING 3 » — et la nature de la place.
//
...
// pistes, Le Bourget ne serait qu'une bouillie de points.
//
// LES TAXIWAYS SONT IGNORÉS — délibérément. SimConnect livre aussi le réseau de
// roulage (TAXI_POINT, TAXI_PATH, TAXI_NAME) ; il n'apporterait rien au brief du
// mercredi soir et alourdirait la base d'un ordre de grandeur.
//
// LA DONNÉE N'EXISTE QU'APRÈS RÉIMPORT. Les places viennent du nœud
// TAXI_PARKING, que l'extraction ne demandait pas avant septembre 2026 : une
// base plus ancienne n'en contient aucune. Ce n'est pas la même chose que
// « pas de place ici », et la couche le dit plutôt que de rester muette.
//
// UN TERRAIN NE SE RÉDUIT PAS À SON POINT DE RÉFÉRENCE. Les places d'une grande
// plate-forme s'en écartent de plusieurs kilomètres : à Roissy, la plus
// lointaine est à 4,2 km. Chercher les terrains sur leur seule référence
// éteignait toutes leurs places dès qu'on s'éloignait du centre — c'est-à-dire
// dès qu'on regardait vraiment quelque chose. L'emprise interrogée est donc
// élargie du rayon MESURÉ de chaque terrain (cf. dansBboxElargi côté main).
// ============================================================
...
const PARKING_COULEUR = '#2563eb';        // bleu de la charte
...
let _parkingsBaseSignalee = false;   // l'avertissement « base sans places » n'est dit qu'une fois
...
async function rafraichirParkings() {
  ...
  if (!layerState.parkings || map.getZoom() < ZOOM_MIN_PARKINGS) { parkingsLayer.clearLayers(); return; }

  const b = map.getBounds();
  const bbox = { south: b.getSouth(), west: b.getWest(), north: b.getNorth(), east: b.getEast() };
  const reqId = ++_parkingsReqId;
  let res;
  try { res = await window.cap.parkingsDansBbox(bbox); } catch (_) { return; }
  if (reqId !== _parkingsReqId) return;

  parkingsLayer.clearLayers();
  if (!res || !res.ok) return;

  // Base antérieure à l'extraction des places : on le dit UNE fois, sinon la
  // couche allumée resterait vide sans qu'on sache pourquoi.
  if (res.baseSansParkings && !_parkingsBaseSignalee) {
    _parkingsBaseSignalee = true;
    if (typeof messageDoc === 'function') messageDoc(t('parkingsBaseAncienne'));
    return;
  }
