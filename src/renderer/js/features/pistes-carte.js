...
// ============================================================
//
// Chaque piste est un RECTANGLE GÉOGRAPHIQUE : ses quatre coins se déduisent
// des deux seuils et de la largeur publiée. Il est donc à l'échelle de la carte
// — il grandit avec le zoom comme le terrain qu'il représente, au lieu d'être
// un symbole de taille fixe. C'est l'écart voulu avec le schéma de piste de
// NavXpressVFR, qui stylise la largeur (8 à 14 px) pour rester lisible dans un
// cadre de 360 px : ici, la carte porte déjà l'échelle, il n'y a rien à styliser.
//
// Ce que ça donne aux zooms utiles, à la latitude de la France (45°) :
//   zoom 12 → 27 m/px   : une piste de 42 m — la largeur médiane en France —
//                         fait 1,6 px. Un trait, mais un trait juste, à sa
//                         longueur et à son orientation exactes.
...
//
// Le tracé est exact et non approché : vérifié sur 401 pistes françaises, la
// distance entre les deux seuils enregistrés vaut la longueur déclarée à
// 0,003 % près (médiane) — les seuils MSFS sont bien les extrémités physiques,
// pas des seuils décalés.
//
...
// appel : allumer la couche coûte ~770 ms et 24 Mo, une fois. Ensuite un
// déplacement de carte coûte ~7 ms — une fenêtre au zoom 12 ne contient guère
// plus d'une dizaine de pistes, même au-dessus de Paris ou de Los Angeles.
//
...
// faute de quoi Roissy s'effacerait dès qu'on regarde le bout de sa 09L.
// ============================================================
...
async function rafraichirPistes() {
  ...
  let res;
  try { res = await window.cap.pistesDansBbox(bbox); } catch (_) { return; }
